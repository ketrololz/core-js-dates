...
function getNextFriday(date) {
  ...
  if (currDay < 5) {
    daysToFriday = 5 - currDay;
  }
  const nextFriday = new Date(date.setDate(date.getDate() + daysToFriday));
  return nextFriday;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const timeDiff = dateEndObj.getTime() - dateStartObj.getTime();
...
function formatDate(date) {
  ...
  const half =
    dateObj.getUTCHours() + dateObj.getUTCMinutes() / 60 > 12 ? 'PM' : 'AM';

  return `${month}/${day}/${year}, ${hours}:${minutes}:${seconds} ${half}`;
}
